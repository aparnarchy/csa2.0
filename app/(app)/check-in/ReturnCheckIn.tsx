  ...
  return (
    <div className="mx-auto flex min-h-[100dvh] w-full max-w-md flex-col bg-lav-bg px-5 pb-8 pt-6">
      <div className="flex flex-1 flex-col justify-center">
        <div className="flex items-center gap-3">
          <div className="min-w-0 flex-1">
            ...
            <h1 className="mt-1 font-display text-[22px] font-black leading-tight text-ink">
              {t.title}
            </h1>
          </div>
          <Mascot state="sad" size={HEADER_MASCOT_SIZE} float={false} sparkle={false} />
        </div>

        {/* recall the recommendation */}
        <div className="mt-5 rounded-card bg-lav-soft p-4">
          <span className="text-[11px] font-semibold text-ink-3">{rec.weekLabel}</span>
          <p className="mt-2 text-xs leading-relaxed text-ink-3">{t.recallIntro}</p>
        ...
        {/* the prompt */}
        <p className="mt-6 font-display text-lg font-black text-ink">{t.prompt}</p>
        ...
        {answer === "not_yet" && (
          <div className="screen-enter mt-5 flex items-start gap-3 rounded-card bg-lav-soft p-4">
      ...
      <button
        ...
        className="w-full rounded-2xl bg-brand py-3.5 font-display text-sm font-black text-white transition active:scale-[0.98] disabled:opacity-40"
      >
